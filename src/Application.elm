...
update msg model =
  case msg of
    ...
    RoomCreation (Ok response) ->
        ...
        ( { model | roomCreated ="https://" ++ model.url.host ++  Maybe.withDefault "" (Maybe.map String.fromInt model.url.port_)++ String.dropRight 16 model.url.path ++ "Room.html?roomID=" ++ model.randomString, roomCreatedString = "Room was created. Visit this Link to join: " }, Cmd.none )
